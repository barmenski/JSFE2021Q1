...
*/
export function getItems(): Promise<ICard[][]> {
  return Promise.resolve<ICard[][]>(items);
}

export function getItemByName(word: string): Promise<ICard | undefined> {
  return Promise.resolve(
    items[0].find((it) => it.word.toLowerCase() === word.toLowerCase())
  );
}
/*
export function createItem(item: Item): Promise<Item> {
  const isExist =
    typeof items.find(
      (it) => it.word.toLowerCase() === item.word.toLowerCase()) !== "undefined";
  if (isExist) {
    return Promise.reject(
      new Error(`Item with name ${item.word} is already exists.`)
    );
  }
  items.push(item);
...
export function updateItem(item: Item): Promise<Item> {
  const itemIndex = items.findIndex(
    (it) => it.word.toLowerCase() === item.word.toLowerCase()
  );
  if (itemIndex < 0) {
