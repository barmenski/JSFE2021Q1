    ...
    onClickHandlerPlay = (item: ICard) => { //item - объект из cardBase.ts
      const starContainer = document.querySelector(".play-btn-container");
        if(this.cardArray[0] === item.audioSrc) {
          ...
          if(starContainer) starContainer.appendChild(starLose);
      ...
      render() {
            ...
            {cards[this.categoryNumber].map((item: ICard) =>{
              return (
                <div className="card" key={item.word} onClick = {this.props.isPlay ?() => this.onClickHandlerPlay(item):() => this.onClickHandlerTrain(item)} onMouseLeave = {()=>this.mouseLeave(item)}>
                  ...
                  <img src="../img/speaker-icon.svg" alt="speaker-icon" className="speaker-icon hidden" data-spk-img={item.word}/>
                </div>
              )
            })
          }
        ...
        <div className="result-container">
          <img src="../img/success.jpg" className = "success__img hidden" alt="Success image"/>
          <img src="../img/failure.jpg" className = "failure__img hidden" alt="Failure image"/>
        </div>
        <Footer />
      </>
      );
